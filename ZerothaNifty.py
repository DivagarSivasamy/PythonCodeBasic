...
def login():
    ...
    data = kite.generate_session("iwr4aSh9BppOkffP13iLy3zbLnHSX9LZ", api_secret="s7jpa4hrrpievt66oxqivewy25pc7qf2")
...
def SuperTrend(df, period=supertrend_period, multiplier=supertrend_multiplier, ohlc=['open', 'high', 'low', 'close']):
    ...
    df = calculateVwap(df)
    writer = pd.ExcelWriter("C:\\Users\\Administrator\\Documents\\testig\\" + 'testunjsdg.xlsx', engine='xlsxwriter')
...
def plcaeFutureNiftyOrder(next_print, buyTradeCount, sellTradeCount, orderTradedCondition):
    if not orderTradedCondition:
        ...
        try:
            ...
            if super_trend_value < EMA21 < EMA9 < vwap < lastclose and buyTradeCount <= 2:
                your_custom_conditions = kite.TRANSACTION_TYPE_BUY
                buyTradeCount = buyTradeCount + 1
            elif super_trend_value > EMA21 > EMA9 > vwap > lastclose and sellTradeCount <= 2:
...
print_interval = datetime.timedelta(minutes=5)
    ...
    while True:
        if (datetime.datetime.now().hour * 60 + datetime.datetime.now().minute) >= orderopentime:
            ...
            if now >= next_print:
                ...
                print(orderTradedCondition)
                print(buyTradeCount)
                print(sellTradeCount)
                print(next_print)
